...
function formatOldThread(site, siteURL, status, character, feature, title, threadID, icDate, partnerObjects, type, lastPost, delayClass, directoryString) {
    ...
    let html = `<div class="thread spy-track grid-item status--${status} ${character.split(' ')[0].toLowerCase()} delay--${delayClass} type--${type.split(' ')[0]} ${tagClasses} ${partnerClasses} grid-item"><div class="thread--wrap">
...
function formatThread(site, siteURL, status, character, feature, title, threadID, icDate, partnerObjects, type, lastPost, delayClass, directoryString, snippet, tags) {
    ...
    if (status !== 'complete' && status !== 'archived') {
        ...
        buttons = `<div class="iconsm" title="${type}" style="width:30px;"></div><button onClick="markArchived(this)" data-id="${threadID}" data-site="${site}" data-character="${character.split('#')[0]}" title="Archive"><i class="fa-regular fa-trash"></i><i class="fa-solid fa-spinner fa-spin"></i></button>`;
    } else {
        buttons = `<div class="icon" title="${type}"></div>`;
    }
    let html = `<div class="thread spy-track grid-item status--${status} ${character.split(' ')[0].toLowerCase()} delay--${delayClass} type--${type.split(' ')[0]} ${tagClasses} ${partnerClasses} ${ftClasses} grid-item">
...
function formatCharacter(data, characterFilters, baseUrl) {
    ...
    let threads;
    if(data.FilterThreads) {
        threads = data.FilterThreads.split('+').map(item => `threads-${item}`).join(' ');
    }
