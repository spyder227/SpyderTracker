...
function formatThread(site, siteURL, status, character, feature, title, threadID, icDate, partnerObjects, type, lastPost, delayClass, directoryString, snippet) {
    ...
    let html = `<div class="thread spy-track grid-item status--${status} ${character.split(' ')[0].toLowerCase()} delay--${delayClass} type--${type.split(' ')[0]} ${partnerClasses} ${ftClasses} grid-item">
...
function formatCharacter(data, characterFilters, baseUrl) {
    ...
    return `<div class="${data.Character.split(' ')[0].toLowerCase()} spy-track grid-item ${characterFilters} canon-${data.FilterCanon} shipped-${data.FilterShipped} ${needs} ${threads} gender-${data.FilterGender} ${ageFilters} ${sitesPlayed} housing-${data.FilterHousing} residence-${data.FilterResidence} homeowner-${data.FilterOwner}">
        <div class="character">
